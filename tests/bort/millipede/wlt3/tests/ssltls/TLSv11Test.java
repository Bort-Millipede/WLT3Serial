/*
	...
	Test to ensure WLT3Serial communicates via T3S with TLSv1.1
*/
...
public class TLSv11Test {
	
	@Test
	public void testAssertUsingTLSv11() {
		...
		WLT3Serial.main(new String[] {"--t3s=TLSv1.1",host,strPort,"CommonsCollections5","curl http://192.168.1.12/"+this.getClass().getName()+"testAssertUsingTLSv11"});
		...
		//Assert that only 1 SSL/TLS protocol is enabled and that it is TLSv1.1
		Assert.assertEquals("Multiple SSL/TLS protocols enabled instead of only TLSv1.1!",1,prots.length);
		Assert.assertEquals(prots[0]+" enabled rather than TLSv1.1","TLSv1.1",prots[0]);
		...
		System.out.println(this.getClass().getName()+"->testAssertUsingTLSv11: WLT3Serial executed successfully using T3S with TLSv1.1");
	}
}
